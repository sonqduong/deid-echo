...
public class EntropyCodedSegment {
	...
	private final int[] blackRedactionDCValue;
...
//System.err.println("maxVerticalSamplingFactor "+maxVerticalSamplingFactor);
		blackRedactionDCValue = buildBlackRedactionDCValues();
...
//System.err.println("rowLength["+c+"] "+rowLength[c]);
	...
	private int[] getQuantizationTableDestinationSelector() throws Exception {
		Field field = MarkerSegmentSOF.class.getDeclaredField("QuantizationTableDestinationSelector");
		field.setAccessible(true);
		return (int[])field.get(sof);
	}

	private int getQuantizationTableElement0(QuantizationTable quantizationTable) throws Exception {
		Field field = QuantizationTable.class.getDeclaredField("QuantizationTableElement");
		field.setAccessible(true);
		int[] elements = (int[])field.get(quantizationTable);
		if (elements == null || elements.length == 0) {
			throw new Exception("Quantization table missing DC coefficient");
		}
		return elements[0];
	}

	private int[] buildBlackRedactionDCValues() throws Exception {
		int[] values = new int[nComponents];
		if (!isDCT) {
			return values;
		}

		int[] quantizationTableDestinationSelector = getQuantizationTableDestinationSelector();
		for (int c=0; c<nComponents; ++c) {
			// A constant black 8x8 block has level-shifted spatial samples of
			// -128, hence an unquantized DC coefficient of -1024. For color
			// JPEGs, keep chroma components neutral and blacken luminance.
			if (nComponents == 1 || c == 0) {
				String key = Integer.toString(quantizationTableDestinationSelector[c]);
				QuantizationTable quantizationTable = qtByIdentifer.get(key);
				if (quantizationTable == null) {
					throw new Exception("Missing quantization table "+key+" for redaction");
				}
				int q00 = getQuantizationTableElement0(quantizationTable);
				values[c] = Math.round(-1024.0f / (float)q00);
			}
			else {
				values[c] = 0;
			}
...
//System.err.println("Redacting - first redaction");
							newDCDIFF = blackRedactionDCValue[c] - originalDCValue[c];	// target black instead of PixelMed's original neutral gray
...
//System.err.println("Not redacting - but was last time, so use diff to restore original value plus specified diff for this block");
							originalDCValue[c] += dcDIFF;
							newDCDIFF = originalDCValue[c] - blackRedactionDCValue[c]; // bridge back from the black redaction target
						}
						else {
